# **Week # Goals**
## Goal 1:
Complete the course [Python Data Structures](https://www.coursera.org/learn/python-data/home/welcome)
## Goal 2:
Read the [python-docs](https://docs.python.org/3/tutorial/) related to topics from course.
## Goal 3:
Solve programs from [hackerrank - python](http://hackerrank.com/domains/python)

---


# Daily Progress:
## Day 1
Nothing
## Day 2

## Day 3
..
## Day 4
..
## Day 5
..
## Day 6
..
## Day 7
.. 

---

## Courses Completed This Week
* []()

About Completed course: 
[README link to the repository]()

---

## Time Spent
* ? hours daily watching video lectures
* ? hours daily on reading material
* ? hours daily Coding
* ? hours daily Time Spent on Project*
* ? hours daily Time spent Daily
Total time daily spend was ? hours a day. 
### How i managed my time
Mostly I would program in the mornings because the mind is fresh at that time and during the rest of the day I would watch the video lectures and solve quizzes for the course. I would read the documentation by the end of the day.
